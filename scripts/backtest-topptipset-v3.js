...
/**
 * backtest-topptipset-v3.js
 *
 * V3 STRATEGY: EV-Ranked Row Selection
 *
 * Key insight: Instead of gardering (structural), enumerate all 6561 rows,
 * rank them by individual EV, and play the top-N rows with positive EV.
 *
 * EV per row = (P_market(row) / P_public(row)) × (1 - houseCut) - 1
 * = edge_product(row) × 0.65 - 1
 *
 * Only play rows where EV > 0, i.e., edge_product > 1.538
 *
 * Additional filter: skip entire draws where no rows have positive EV.
 */
...
function buildConfigGrid() {
  ...
  // minEdgeProduct thresholds (must be > 1/0.65 = 1.538 for positive EV)
  const minEdgeProducts = [1.538, 1.6, 1.7, 1.8, 2.0, 2.5];
  const maxRowsList = [5, 10, 20, 50, 100, 200];
...
async function main() {
  ...
  const now = new Date();
  const runLabel = `topptipset-v3-${now.toISOString().replace(/[^0-9]/g, '').slice(0, 14)}`;

  console.log(`\n=== Topptipset V3 EV-Ranked Row Selection ===`);
